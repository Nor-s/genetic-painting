...
namespace nsg
{
    ...
    */
    Picture::Picture(int width, int height, int byte_per_pixel, GLenum pixel_format)
    {
        ...
        int stride_size_ = get_relative_width() * byte_per_pixel_ + padding_;
        image_size_ = stride_size_ * get_relative_height();

        image_data_ = new GLubyte[image_size_];
        memset(image_data_, 255, image_size_ * sizeof(*image_data_));
        SquareObject::init_texture((GLvoid *)image_data_, relative_width_, relative_height_, pixel_format_);
    ...
    Picture::Picture(const char *filepath, int byte_per_pixel)
    {
        ...
        image_data_ = nullptr;
        SquareObject::set_vertices();
        std::cout<<"-- tex: "<<tex_height_<<" "<<tex_width_<<"\n";
        init_buffer_objects();
        SquareObject::init_shader();
        SquareObject::translate(0.0f, 0.0f, -50.0f);
        SquareObject::set_model_to_uniform();
    ...
    {
        read_back_buffer(0, 0, get_relative_width(), get_relative_height());
    }
    void Picture::read_back_buffer(int posx, int posy, int width, int height)
    {
        glReadPixels(
            posx, posy,
            width, height,
    ...
    {
        start_sub_picture(0, 0, relative_width_, relative_height_);
    }
    void Picture::start_sub_picture(int posx, int posy, int width, int height)
    {
        read_back_buffer(posx, posy, width, height);
        unbind_write_pbo();
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, posx, posy, width, height, pixel_format_, GL_UNSIGNED_BYTE, 0);
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
    }
